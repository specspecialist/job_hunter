...
exports.handler = async function(event) {
  try {
    ...
    try {
      ...
      return { statusCode: 401, body: JSON.stringify({ error: 'Invalid id_token' }) };
    }
    ...
    if (ALLOWED_EMAILS.length > 0 && !ALLOWED_EMAILS.includes(email)) {
      return { statusCode: 403, body: JSON.stringify({ error: 'Email not allowed' }) };
    }
    if (ALLOWED_DOMAIN && hd && ALLOWED_DOMAIN !== hd) {
      return { statusCode: 403, body: JSON.stringify({ error: 'Google Workspace domain not allowed' }) };
    }
